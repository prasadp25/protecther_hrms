...
const FnFSettlement = ({ employeeId, onBack }) => {
  ...
  // ---- Settlement view ----
  const net = Number(settlement.net_payable || 0);
  ...
  const LineRows = ({ kind }) => {
    ...
    return (
      <div>
        ...
        {isDraft && (
          <button onClick={() => addManual(kind)} className="text-blue-600 text-sm mt-1">+ Add line</button>
        )}
      </div>
    );
  };
    ...
    <div className="p-6 max-w-4xl">
      ...
      <div className={`mt-4 p-4 rounded border-2 ${recoverable ? 'border-red-300 bg-red-50' : 'border-slate-800 bg-slate-50'}`}>
        <div className="flex justify-between items-center">
          <span className="font-bold">{recoverable ? 'NET RECOVERABLE FROM EMPLOYEE' : 'NET F&F PAYABLE'}</span>
      ...
      <div className="mt-4 bg-gray-50 border rounded p-3 text-sm">
        ...
        <div className="flex justify-between"><span>Statutory gratuity estimate (15/26 × basic × yrs)</span><span>{formatCurrency(settlement.ref_statutory_gratuity)}</span></div>
        <p className="text-xs text-gray-500 mt-2">
          Gratuity and statutory bonus are folded into monthly CTC and disbursed with each payslip,
          so they are not added as a lump sum here (see spec §2).
        </p>
      </div>
